...
## Purpose
This guide helps to upgrade the `sdtooling-api-be` release from the previous one.
...
- dependencies:
	- asset orchestrator version >= 0.2.0
	- edc connector adapter version >= 1.15.0
	- sd schemas version >= 1.12.0

- values added:

	```
	# asset-orchestrator application
	assetOrchestrator:
	  service:
	    # URL to the Asset Orchestrator participant service
	    url: "http://asset-orchestrator:8080"
	```
