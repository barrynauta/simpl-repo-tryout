...
## Purpose
This guide helps to upgrade the `xfsc-advsearch-be` release from the previous one.

---

## Breaking Changes
- Deprecated API `/v1/schemas` has been removed.
...
- Added ArgoCD annotation Sync wave to values.yaml:
    ...
    argocd:
      syncWave: "1"
        # Sync wave for xfsc-advsearch-be in argocd
    ```
