...
| `repository_url` | string | URL | Canonical source-code URL. |
...
6. **Cleanup pass on non-solution README frontmatter.** The cleanup pass in this session stripped frontmatter from *solution* README files only. Other README types (dimensions, capabilities, business services, BPs, NFRs, indexes) still carry registry-spec frontmatter (`id`, `type`, `name`, `since`) per `foundations/registry-spec.md`. Whether that frontmatter should also be removed depends on whether the registry-spec id system stays as a separate mechanism or is folded into the manifest. Defer to the maintainer.
