...
The page can be accessed via the following link `<authority-frontend>/identity-provider/onboarding-requests`.
...
The page can be accessed via the following link `<authority-frontend>/identity-provider/onboarding-requests/:requestId`.
(i.e., a user with the **NOTARY** role, such as the preconfigured user `m.t`)
...
The notary can also view the detail of the eIDAS documents uploaded by the applicant, by clicking the (View eIDAS Data) button of an eIDAS document.
There are no specific links to view the window, as this feature is available from the onboarding request details page.

![](./imgs/onboarding/onboarding-notary/onboarding%20detail%20documents.png)
![](./imgs/onboarding/onboarding-notary/notary%20view%20eidas.png)
