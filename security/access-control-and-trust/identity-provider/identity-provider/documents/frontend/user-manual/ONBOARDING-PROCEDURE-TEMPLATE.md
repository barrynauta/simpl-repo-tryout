...
2. [Onboarding procedure template - documents](#2-onboarding-procedure-template---documents)
   1. [Manage documents](#21-manage-documents)
      1. [Manage User upload documents](#211-manage-user-upload-documents)
      1. [Manage MIME Types](#212-manage-mime-types-for-documents)
      2. [Manage eIDAS documents](#213-manage-eidas-documents)
...
You can create and manage these templates by navigating to the following link: `<authority-frontend>/identity-provider/onboarding-templates` logging in as a user with the T2IAA_M role, such as the preconfigured user e.j@email.com.
...
The documents section is available by navigating to the following link: `<authority-frontend>/identity-provider/onboarding-templates` logging in as a user with the T2IAA_M role, such as the preconfigured user e.j@email.com.
...
To add your first document, click on the (Add Document) button. This action will open a modal that allows you to select the document type from a drop-down list.

### 2.1.1 Manage User Upload Documents
...
### 2.1.2 Manage MIME types for documents
...
### 2.1.3 Manage eIDAS Documents

![](./imgs/onboarding/onboarding-procedure-template/onboarding%20procedure%20template%20add%20document%20eidas.png)

The system will automatically pre-populate some example data for the eIDAS documents. All fields are required. 
...
Onboarding procedure template validation rules can be accessed via the following link `<authority-frontend>/identity-provider/onboarding-templates` logging in as a user with the T2IAA_M role, such as the preconfigured user e.j@email.com) 
...
eIDAS rules are only available if the frontend component is configured to use eIDAS and there is at least one eIDAS document configured.

![](./imgs/onboarding/onboarding-procedure-template/onboarding%20procedure%20template%20eidas%20disabled.png)
