...
14. [onboarding Requests Management](#14-onboarding-requests-management)
15. [onboarding Templates Management](#15-onboarding-templates-management)
...
To suspend credentials, you can either use the action from the table shown on the participants page ([see this section](#1-participant-list)) or click on the button (Suspend credentials) on the details page. In the confirmation window that appears on the screen, enter the reason for performing the operation. In the confirmation window that appears on the screen, enter the reason for performing the operation. To confirm the operation, click OK in the warning window or cancel.
...
To revoke a credential, use the Revoke credential button accessible from the participant's detail page. A warning window will open when you click the button, compile the reason for revoke. To continue, press OK; otherwise, cancel.
...
To renew your credentials, go to the participant details page and click on the appropriate button (Renew credential). A window will appear on the screen with input text reason. Select Renew button to confirm the operation or cancel.
...
You can enable or disable automatic renewal using the toggle (Auto renewal enabled). When this is active, you can decide whether to use the default value for that participant or configure a custom one. By moving the second toggle (Use default settings), you tell the system to use the preconfigured setting. Conversely, by disabling the second toggle, you must enter a number of days between 0 and 1830. You can specify the reason for changing the configuration by entering text in the input field. To confirm, click on the save button; otherwise, you can reset to the initial settings.

![](./imgs/identity-provider/participant%20detail%20auto%20renewal%20settings.png)

### 14. Onboarding Requests Management

Please refer to the [Onboarding Requests Management](./ONBOARDING-NOTARY.md) section.

### 15. Onboarding Templates Management

Please refer to the [Onboarding Templates Management](./ONBOARDING-PROCEDURE-TEMPLATE.md) section.
